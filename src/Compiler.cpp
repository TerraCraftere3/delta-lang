...
namespace Delta
{
    ...
    int Compiler::compile(const CompilerProperties &props)
    {
        ...
        if (props.inputFiles.empty())
        {
            LOG_ERROR("No input files provided");
            return 1;
        }

        LOG_INFO("Compiling {} file(s)...", props.inputFiles.size());
        ...
        for (const auto &inFile : props.inputFiles)
        {
            ...
            // Per-file include: add input directory at front for this pass
            std::vector<std::string> perFileIncludeDirs = includeDirs;
            perFileIncludeDirs.insert(perFileIncludeDirs.begin(), Files::getDirectory(inputPath));
            ...
            std::vector<Token> processedTokens = processor.process(perFileIncludeDirs).tokens;
            ...
            if (parseTree.has_value())
            {
                ...
                std::string fileAssembly = assembler.generate();
                fileAssembly = assemblyPrefix + fileAssembly;
        ...
        // If only compiling, we're done after creating object files
        if (props.compileType == COMPILE_ONLY)
        {
            LOG_INFO("Compiled {} translation unit(s) to objects in {}", objectFiles.size(), intDir);
            return 0;
        }
        ...
        if (props.compileType == COMPILE_LINK_AND_RUN)
        {
            if (isWasm)
            {
